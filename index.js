...
var extraOptions = [ //all of these values must be under globalVariables as well
    'myID', 'myName', 'botID', 'ignoreArray', 'welcomeMessage'
]
...
function enableFeatures(options) {
    ...
    if (firstRun) { ///first run, in case you use enableFeatures multiple times
        ...
        setInterval(function() {
            ab.dataCollection.acceptPendingThreads(api);
        }, 60000);
    }
...
function listen(message) {
    ...
    try {
        ...
        if (v.godMode) log.info('user has godMode');

        v.devMode = v.contains(devArray, message.senderID) || v.godMode;
        if (v.devMode && !v.godMode) log.info('user has devMode');
        ...
        if (v.godMode) {
            ...
            if (v.continue && input) ab.dataCollection.collect(api, message, input);
